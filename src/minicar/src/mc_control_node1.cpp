...
//yooooooooooooooooooo start
  void ControlNode::calc2MotorCommands_withOdometry()
  {
    ...
    geometry_msgs::PointStamped minicar_odom;
    minicar_odom.header.stamp = ros::Time::now();
    minicar_odom.header.frame_id = "minicar_odom_phi_x,y_dot";
    minicar_odom.point.x = positionxyz.at(0) ;
    minicar_odom.point.y = positionxyz.at(1) ;
    minicar_odom.point.z = theta_;
    calc_minicar_odom_pub_.publish(minicar_odom);
	...
	VR = //0.5*C_theta_-0.5*C_y_;
             1.0*(gains_2D_Kxz_[1]*imu_phi_.at(1)+gains_2D_Kxz_[3]*imu_dphi_.at(1)+gains_2D_Kxz_[0]*(cos(theta_)*(positionxyz.at(0)-0.1*t_)
             +sin(theta_)*(positionxyz.at(1)-0.1*t_)-(I/0.035)*(theta_-3.14159265359/40*t1_))       
  ...
  ControlNode::ControlNode(ros::NodeHandle& nh)
  {
    ...
    //Subscriber:
    imu_sub_ = nh.subscribe("/minicar/sensor/imu", 5, &ControlNode::imuCallback,this);
    joints_sub_ = nh.subscribe("/minicar/joints/joint_states", 5, &ControlNode::jointsCallback,this);
    ...
    joint_commands_1_pub_ = nh.advertise<std_msgs::Float64>("/minicar/joints/rwheel_"+motors_controller_type_+"_controller/command", 5);
    joint_commands_2_pub_ = nh.advertise<std_msgs::Float64>("/minicar/joints/lwheel_"+motors_controller_type_+"_controller/command", 5);

    //Publish rpy angles:
    rpy_pub_ =  nh.advertise<geometry_msgs::PointStamped>("rpy_angles", 5);
    calc_minicar_odom_pub_ = nh.advertise<geometry_msgs::PointStamped>("minicar", 5);
...
{
  ros::init(argc, argv, "minicar_control");
  ROS_INFO("yooooooooooooooostart minicar_control_node:");
  ...
  minicar::ControlNode node(nh);
  ros::Rate loop_rate(nh.param("/minicar/control_constants/update_rate", 100.0));
